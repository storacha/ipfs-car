...
export async function packToFs ({ input, output, blockstore: userBlockstore, unixfsImporterOptions }: PackToFsProperties) {
  ...
  const { root } = await packToStream({ input, writable, blockstore, unixfsImporterOptions })
